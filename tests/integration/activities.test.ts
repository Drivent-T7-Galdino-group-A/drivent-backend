...
describe("GET /activities/activity-tickets/:activityId", () => {
  it("should respond with status 401 if no token is given", async () => {
    const response = await server.get("/activities/activity-tickets");
  ...
  it("should respond with status 401 if given token is not valid", async () => {
    ...
    const response = await server.get("/activities/activity-tickets").set("Authorization", `Bearer ${token}`);
  ...
  it("should respond with status 401 if there is no session for given token", async () => {
    ...
    const response = await server.get("/activities/activity-tickets/1").set("Authorization", `Bearer ${token}`);
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 400 when activityId was not a number", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      await createTicket(enrollment.id, ticketType.id, TicketStatus.RESERVED);
      await createHotel();
      const activityId = faker.lorem.word();

      const response = await server.get(`/activities/activity-tickets/${activityId}`).set("Authorization", `Bearer ${token}`);

      expect(response.status).toEqual(httpStatus.BAD_REQUEST);
    });

    it("should respond with status 200 and the number of enrollments by activity with given activityId", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      ...
      const response = await server
        .get(`/activities/activity-tickets/${activityTicket.activityId}`)
        .set("Authorization", `Bearer ${token}`);
    ...
    it("should respond with status 200 and the number of enrollments by activity when there is no activityTicket with given activityId", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
